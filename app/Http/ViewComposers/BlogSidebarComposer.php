...
class BlogSidebarComposer
{
    ...
    private function __getCategories()
    {
        return BlogCategory::select([
                                        ...
                                        'name',
                                        'position',
                                    ])->withCount('articles')->having('articles_count', '>',
                                                                      0)->orderBy('position')->get()->toArray();
    }
    ...
    {
        return BlogTag::select([
                                   'id',
                                   'tag',
                               ])->withCount('articles')->having('articles_count', '>',
                                                                 0)->orderByDesc('articles_count')->orderByDesc('id')->take(15)->get()->toArray();
    }
    ...
    private function __getLatestPosts()
    {
        return BlogArticle::select([
                                       ...
                                       'created_at',
                                       'slug',
                                   ])->without('tags')->wherePublished(true)->orderByDesc('created_at')->take(4)->get()->toArray();
    }
